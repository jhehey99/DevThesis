...
time = np.linspace(time[0], time[-1], num=500)
value = fc(time)
...
new2_inflct_points = [new_inflct_points[i] for i in range(len(new_inflct_values)) if
                      new_inflct_values[i] >= ave_ifv_low]
...
for i in range(len(final_systolic_times)):
    if final_systolic_times[i] < final_diastolic_times[i]:
        final_systolic_points.pop(i);
        final_systolic_times.pop(i);
        final_systolic_values.pop(i);
    else:
        break
...
for i in range(len(final_diastolic_times)):
    if final_diastolic_times[-1] > final_systolic_times[-1]:
        final_diastolic_points.pop();
        final_diastolic_times.pop();
        final_diastolic_values.pop();
    else:
        break
...
if len(final_diastolic_times) > 0:
    ...
    if last_match_spi != -1:
        while last_match_spi < len(final_systolic_points):
            ...
            final_systolic_points.pop();
            final_systolic_times.pop();
            final_systolic_values.pop();
            last_match_spi += 1
...
if len(final_systolic_times) > 0:
    ...
    if first_match_dpi != -1:
        while first_match_dpi > 0:
            final_diastolic_points.pop(0);
            final_diastolic_times.pop(0);
            final_diastolic_values.pop(0);
            first_match_dpi -= 1
...
plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
plt.plot(time, grad22, color="g", linestyle="dashed")
# plt.plot(time, grad11, color="r", linestyle="dashed")
plt.plot(time, grad33, color="r", linestyle="dashed")
